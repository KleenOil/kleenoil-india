...
export async function CsFeaturedBlock({ block }: CsFeaturedBlockProps) {
  ...
  const eyebrow = cmsText(block?.eyebrow, DEFAULT_CS_FEATURED.eyebrow, hasCms);
  const heading = cmsText(block?.heading, DEFAULT_CS_FEATURED.heading, hasCms);
  const description = cmsText(block?.description, DEFAULT_CS_FEATURED.description, hasCms);
  ...
  const fallback = hasCms ? null : DEFAULT_CS_FEATURED.study;
  const title = card?.title ? study?.featuredHeadline?.trim() || card.title : fallback?.title || '';
  const tag = (card?.tag || fallback?.tag || '').replace(/^CASE STUDY \/ /i, '');
  const location = card?.location || fallback?.location || '';
  const excerpt = card?.description || fallback?.excerpt || '';
  const imageUrl = card?.imageUrl || fallback?.imageUrl || '';
  const imageAlt = card?.imageAlt || title;
  const href = card?.href || fallback?.href || '';
  const openPdf = Boolean(card?.openInNewTab);
  const metrics = card?.metrics.length ? card.metrics : (fallback?.metrics ?? []);
