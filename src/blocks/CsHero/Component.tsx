...
export function CsHeroBlock({ block }: CsHeroBlockProps) {
  const eyebrow = block?.eyebrow || DEFAULT_CS_HERO.eyebrow;
  const heading = block?.heading || DEFAULT_CS_HERO.heading;
  const subheadline = block?.subheadline || DEFAULT_CS_HERO.subheadline;
  const watermark = block?.watermark || DEFAULT_CS_HERO.watermark;
  const cta = resolveLink(block?.cta) ?? DEFAULT_CS_HERO.cta;
  const imageUrl = getMediaUrl(block?.image) || DEFAULT_CS_HERO.imageUrl;
